...
const WorkoutDisplay: React.FC<WorkoutDisplayProps> = ({ plan }) => {
  ...
  const handleGenerateVideo = async (exerciseIndex: number) => {
    ...
    try {
      // FIX: This now returns a YouTube search URL (always works) instead of
      // a hallucinated embed URL (which was causing "Tutorial not found" errors)
      ...
      setExercises(prev =>
        prev.map((ex, i) =>
          i === exerciseIndex
            ? { ...ex, isVideoLoading: false, videoError: 'Tutorial not found.' }
            : ex
        )
      );
    }
  ...
  return (
    <section className="space-y-12">
      <div className="p-8 bg-neutral-900/20 border-l-4 border-emerald-500 rounded-r-2xl">
        <h3 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest-custom mb-3">
      ...
      <div className="space-y-6">
        {exercises.map((exercise, index) => (
          <div
            key={index}
            className="bg-neutral-900/30 border border-neutral-800/60 rounded-3xl p-6 sm:p-10 transition-all hover:bg-neutral-900/40"
          >
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-10">
              <div className="max-w-xl">
                <div className="flex items-center gap-3 mb-4">
                  <span className="flex items-center justify-center w-6 h-6 rounded-full bg-emerald-500/10 text-emerald-500 text-[10px] font-black border border-emerald-500/20">
                    {index + 1}
                  </span>
                  <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest-custom">
                    Exercise {index + 1}
                  </span>
                </div>
                <h3 className="text-4xl font-black text-white tracking-tighter-custom mb-4 uppercase">
                  {exercise.name}
                </h3>
              ...
              <div className="flex gap-3">
                <div className="flex-1 min-w-[100px] px-4 py-3 rounded-xl bg-black/40 border border-neutral-800 text-center">
                  <span className="text-[9px] font-black text-neutral-600 uppercase tracking-widest block mb-0.5">
                    Sets
                  </span>
            ...
            <div className="grid lg:grid-cols-2 gap-10">
              ...
              <div className="relative group/video">
                {exercise.videoUrl ? (
                  // FIX: Instead of an iframe (which broke with hallucinated URLs),
                  // show a button that opens YouTube search in a new tab. Always reliable.
                  <a
                    href={exercise.videoUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex flex-col items-center justify-center gap-3 w-full h-full min-h-[180px] rounded-2xl bg-neutral-900/50 border border-emerald-500/30 hover:border-emerald-500/60 hover:bg-neutral-800/50 transition-all duration-300 group"
                  >
                    <div className="p-3 rounded-full bg-emerald-500/10 text-emerald-500">
                      <VideoCameraIcon className="h-6 w-6" />
                    </div>
                    <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest-custom">
                      Search on YouTube ↗
                    </span>
                  ...
                  >
                    {exercise.isVideoLoading ? (
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <span className="text-[9px] font-black text-neutral-500 uppercase tracking-widest text-center">
                          Finding Tutorial...
                        </span>
                      </div>
                    ) : (
                      <>
                        ...
                        <span className="text-[10px] font-black text-neutral-400 uppercase tracking-widest-custom group-hover:text-neutral-200">
                          Find Tutorial
                        </span>
                      </>
                    )}
                ...
                {exercise.videoError && (
                  <p className="absolute bottom-4 left-0 right-0 text-center text-[10px] font-bold text-red-500 uppercase tracking-widest">
                    {exercise.videoError}
                  </p>
                )}
              </div>
