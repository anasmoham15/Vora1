...
export default function App() {
  ...
  useEffect(() => {
    ...
    // FIX: Store the subscription so we can unsubscribe when the component unmounts
    // (previously this was leaking memory and causing duplicate event handlers)
  ...
  const handleLoginSuccess = (email: string) => {
    // The onAuthStateChange listener above handles updating state.
    // This callback just closes the modal, which AuthModal calls onSuccess.
    console.log('Login confirmed for:', email);
  };

  const handleLogout = async () => {
    await supabase.auth.signOut();
    // onAuthStateChange will fire and clear user + savedWorkouts automatically
    setMode(null);
  };
    ...
    switch (mode) {
      case 'workout':
        return (
          <WorkoutGenerator
            user={user}
            onSave={(sw) => setSavedWorkouts([sw, ...savedWorkouts])}
          />
        );
      case 'health':
        return <HealthCheck />;
